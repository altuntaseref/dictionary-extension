...
// .wrangler/tmp/bundle-bw783O/strip-cf-connecting-ip-header.js
...
async function requireAuth(request, env2) {
  ...
  const userId = await verifyJwtAndGetUserId(token, env2);
  return { userId };
}
...
async function verifyJwtAndGetUserId(jwt, env2) {
  const url = `${env2.SUPABASE_URL}/auth/v1/user`;
...
var init_auth = __esm({
  "src/middleware/auth.ts"() {
    ...
    __name(verifyJwtAndGetUserId, "verifyJwtAndGetUserId");
  }
});
...
var init_fetch = __esm({
  "node_modules/@supabase/storage-js/dist/module/lib/fetch.js"() {
    ...
    handleError = /* @__PURE__ */ __name((error3, reject, options) => __awaiter(void 0, void 0, void 0, function* () {
...
var init_fetch2 = __esm({
  "node_modules/@supabase/storage-js/dist/module/lib/vectors/fetch.js"() {
    ...
    handleError2 = /* @__PURE__ */ __name((error3, reject, options) => __awaiter(void 0, void 0, void 0, function* () {
...
var init_constants5 = __esm({
  "node_modules/@supabase/auth-js/dist/module/lib/constants.js"() {
    ...
    DEFAULT_HEADERS4 = { "X-Client-Info": `gotrue-js/${version5}` };
...
function getServiceClient(env2) {
  const supabase = createClient(env2.SUPABASE_URL, env2.SUPABASE_SERVICE_ROLE_KEY, {
...
var init_supabase = __esm({
  "src/services/supabase.ts"() {
    ...
    init_performance2();
    init_module5();
    __name(getServiceClient, "getServiceClient");
  }
});

// src/services/llm.ts
async function generateMeaning(word, env2, options) {
  const source = options?.sourceLang?.trim();
  const target = options?.targetLang?.trim() || "Turkish";
  const prompt = source ? `Translate the following ${source} word or phrase into ${target}.
Answer ONLY with the translation (no quotes, no extra text):
"${word}"` : `Detect the language of the following word or phrase and translate it into ${target}.
Answer ONLY with the translation (no quotes, no extra text):
"${word}"`;
  const result = await callOpenAI(prompt, env2);
  return result.trim().replace(/^"|"$/g, "");
}
async function generateExamples(word, env2, options) {
...
Return ONLY a JSON array of exactly 2 strings, nothing else. Example format: ["Sentence 1.", "Sentence 2."]`;
  ...
  return result.split(/\n+/).map((s) => s.trim().replace(/^[-•\d.]+\s*/, "")).filter((s) => s.length > 0 && !s.match(/^(sentence|example)/i)).slice(0, 2);
}
async function callOpenAI(prompt, env2) {
  ...
  if (env2.OPENAI_API_KEY) {
    ...
    if (!res2.ok) {
      ...
      throw new Error(`OpenAI error: ${res2.status} ${text}`);
    }
...
var init_llm = __esm({
  "src/services/llm.ts"() {
    ...
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    __name(generateMeaning, "generateMeaning");
    __name(generateExamples, "generateExamples");
    __name(callOpenAI, "callOpenAI");
  }
});

// src/controllers/translate.ts
var translate_exports = {};
__export(translate_exports, {
  handleTranslate: () => handleTranslate
});
async function handleTranslate(request, env2) {
  try {
    const { userId } = await requireAuth(request, env2);
    const body = await request.json().catch(() => ({}));
    const word = typeof body?.word === "string" ? body.word.trim() : "";
    const sourceLang = typeof body?.source_lang === "string" ? body.source_lang.trim() : void 0;
    const targetLang = typeof body?.target_lang === "string" ? body.target_lang.trim() : void 0;
    if (!word) {
      return jsonError(400, "invalid_request", "'word' is required");
    }
    const meaning = await generateMeaning(word, env2, { sourceLang, targetLang });
    const supabase = getServiceClient(env2);
    const { error: error3 } = await supabase.from("words").insert({
      user_id: userId,
      word,
      meaning
    });
    if (error3) {
      return jsonError(500, "db_error", error3.message);
    }
...
async function handleExample(request, env2) {
  try {
    const { userId } = await requireAuth(request, env2);
    const body = await request.json().catch(() => ({}));
    const word = typeof body?.word === "string" ? body.word.trim() : "";
    const targetLang = typeof body?.target_lang === "string" ? body.target_lang.trim() : void 0;
    if (!word) {
      return jsonError2(400, "invalid_request", "'word' is required");
    }
    const supabase = getServiceClient(env2);
    const { data, error: findErr } = await supabase.from("words").select("id, examples").eq("user_id", userId).eq("word", word).limit(1).maybeSingle();
    if (findErr)
      return jsonError2(500, "db_error", findErr.message);
    if (!data)
      return jsonError2(404, "not_found", "Word not found");
    const examples = await generateExamples(word, env2, { targetLang });
    const merged = Array.isArray(data.examples) ? [...data.examples, ...examples] : examples;
    const { error: updErr } = await supabase.from("words").update({ examples: merged }).eq("id", data.id).eq("user_id", userId);
    if (updErr)
      return jsonError2(500, "db_error", updErr.message);
    return json2({ examples });
  } catch (e) {
    if (e instanceof Response)
      return e;
    return jsonError2(500, "internal_error", e?.message || "Unexpected error");
  }
}
...
var init_example = __esm({
  "src/controllers/example.ts"() {
    ...
    init_supabase();
    init_llm();
    __name(handleExample, "handleExample");
    __name(json2, "json");
...
// src/controllers/export.ts
var export_exports = {};
__export(export_exports, {
  handleExport: () => handleExport
});
async function handleExport(request, env2) {
  try {
    const { userId } = await requireAuth(request, env2);
    const url = new URL(request.url);
    const format = (url.searchParams.get("format") || "json").toLowerCase();
    const supabase = getServiceClient(env2);
    const { data, error: error3 } = await supabase.from("words").select("id, user_id, word, meaning, examples, notes, created_at").eq("user_id", userId).order("created_at", { ascending: true });
    if (error3)
      return jsonError3(500, "db_error", error3.message);
    if (format === "csv") {
    ...
    if (format !== "json") {
      return jsonError3(400, "invalid_format", "format must be 'json' or 'csv'");
    }
    return json3({ words: data || [] });
  } catch (e) {
    if (e instanceof Response)
      return e;
    return jsonError3(500, "internal_error", e?.message || "Unexpected error");
  }
}
...
function jsonError3(status, code, message) {
  return json3({ error: { code, message } }, { status });
}
...
// .wrangler/tmp/bundle-bw783O/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-bw783O/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
async function route(request, env2, ctx) {
  ...
  if (path === "/api/translate" && request.method === "POST") {
    const { handleTranslate: handleTranslate2 } = await Promise.resolve().then(() => (init_translate(), translate_exports));
    return handleTranslate2(request, env2, ctx);
  }
  if (path === "/api/example" && request.method === "POST") {
    const { handleExample: handleExample2 } = await Promise.resolve().then(() => (init_example(), example_exports));
    return handleExample2(request, env2, ctx);
  }
  if (path === "/api/export" && request.method === "GET") {
    const { handleExport: handleExport2 } = await Promise.resolve().then(() => (init_export(), export_exports));
    return handleExport2(request, env2, ctx);
  }
  return json4({ error: { code: "not_found", message: "Route not found" } }, { status: 404 });
}
__name(route, "route");
var src_default = {
  fetch: (request, env2, ctx) => withCors(route)(request, env2, ctx)
};
...
var jsonError4 = /* @__PURE__ */ __name(async (request, env2, _ctx, middlewareCtx) => {
  try {
    return await middlewareCtx.next(request, env2);
  } catch (e) {
